...
type HttpServer struct {
	Handlers           *ControllerRegister //实现http.Handler接口,内部是一个路由管理器,实现了前缀树路由
	Server             *http.Server        //标准库的Server
	Cfg                *Config
...
func NewHttpServerWithCfg(cfg *Config) *HttpServer { //也可以使用带自定义配置的Server
...
func (app *HttpServer) Run(addr string, mws ...MiddleWare) {
	...
	app.Server.Handler = app.Handlers //替换掉标准库的DefaultServeMux,路由也是web框架最为核心的一个部分
	...
	// run normal mode
	if app.Cfg.Listen.EnableHTTPS || app.Cfg.Listen.EnableMutualHTTPS { //是否开启HTTPS
		go func() {
			time.Sleep(1000 * time.Microsecond)
			if app.Cfg.Listen.HTTPSPort != 0 {
	...
	if app.Cfg.Listen.EnableHTTP {
		go func() {
			...
			if app.Cfg.Listen.ListenTCP4 {
				...
				if err := app.Server.ListenAndServe(); err != nil { //标准库的HTTP Server开启监听
